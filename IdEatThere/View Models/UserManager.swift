...
//    }
    ...
    func userCreatesGroup(completion: @escaping (Error?) -> Void) {
        ...
        // Create path to subcollection
        let subCollectionPath = db.collection("users").document(usersDocumentId).collection("activeGroups")
        ...
        // Set path to document in subcollection with the new groupId
        ...
        // Create the new group path collection
        let groupPath = db.collection("groups").document(groupId)

        groupPath.setData(["groupName" : "SubCollectionTest",
                           "attendees" : FieldValue.arrayUnion(["dummyId"]),
                           "host" : FieldValue.arrayUnion(["\(self.userId)"])]) { (error) in
    ...
    func userAddedRestauranttoGroup(groupId: String, restaurantName: String, completion: @escaping (Bool) -> Void) {
